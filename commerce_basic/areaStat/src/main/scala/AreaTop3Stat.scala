...
object AreaTop3Stat {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // RDD[(cityId, CityAreaInfo(cityId, cityName, area))]
    val cityId2AreaInfoRDD = getCityAreaInfo(sparkSession)

    // tmp_area_basic_info(city_id,city_name,area,pid): 表中的一条数据就代表一次点击商品的行为
    ...
    //from tmp_area_basic_info -> tmp_area_click_count(area, pid, city_infos, count(*))
    ...
    //from tmp_area_click_count + product_info ->
    //  tmp_area_count_product_info(area, city_infos, pid, product_name, product_status,click_count)
